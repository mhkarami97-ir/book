...
تست‌های بزرگ (مانند Integration Tests و End-to-End Tests) برای بررسی همین موضوع هستند: **آیا قطعات با هم به درستی کار می‌کنند؟**
...
سیستم‌ها، کتابخانه‌ها و APIهای قدیمی باید روزی کنار بروند تا جای خود را به نسخه‌های جدیدتر، امن‌تر و سریع‌تر بدهند. به این فرآیند **Deprecation** می‌گویند.
...
با وجود میلیاردها خط کد در مخزن یکپارچه (Monorepo)، هیچ مهندسی نمی‌تواند همه چیز را بداند. وقتی می‌خواهید بدانید «کلاس `User` کجا تعریف شده؟» یا «چه کسانی از تابع `Login` استفاده می‌کنند؟»، `grep` کردن ساده دیگر جواب نمی‌دهد.
...
وقتی سازمانی مثل گوگل با میلیاردها خط کد در یک مخزن واحد (Monorepo) کار می‌کند، ابزارهای عادی جست‌وجو (مثل `grep` یا جست‌وجوی ساده IDE) به هیچ وجه پاسخگو نیستند.
...
بسیاری از کامپایلرها (مثل کامپایلر جاوا یا C++) اخطارهایی (Warnings) می‌دهند که نوعی آنالیز استاتیک محسوب می‌شوند. اما گوگل یک گام فراتر رفته است.
...
CI سیستمی است که به طور مداوم کدهای جدید را با کدهای قدیمی ترکیب می‌کند و تست‌ها را اجرا می‌کند تا مطمئن شود چیزی خراب نشده است.
...
در گذشته، نرم‌افزارها هر چند ماه یک‌بار در قالب «نسخه جدید» منتشر می‌شدند. اما در گوگل، سرویس‌هایی مثل جست‌وجو یا جیمیل ممکن است روزانه چندین بار آپدیت شوند.
